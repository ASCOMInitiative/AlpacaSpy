...
{
    public class ProxyObservingConditions : IObservingConditionsV2, IDisposable
    {
        ...
        public ProxyObservingConditions(ConfiguredDevice config, State state, Settings settings, AlpacaSpyLogger logger)
        {
            _config = config;
            _state = state;
            _settings = settings;
        ...
        internal void ConnectDevice()
        {
            ...
            DisconnectDevice();
            _client = new AlpacaObservingConditions(
