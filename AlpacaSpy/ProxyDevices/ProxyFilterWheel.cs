...
namespace AlpacaSpy.ProxyDevices
{
    public class ProxyFilterWheel : IFilterWheelV3
    {
        ...
        public ProxyFilterWheel(ConfiguredDevice config, State state, AlpacaSpyLogger logger)
        {
            _config = config;
            _state = state;
            _logger = logger;
        ...
        internal void ConnectDevice()
        {
            ...
            DisconnectDevice();
            _client = new AlpacaFilterWheel(ASCOM.Common.Alpaca.ServiceType.Http, _config.IpAddress, _config.PortNumber, _config.RemoteDeviceNumber, false, null);
