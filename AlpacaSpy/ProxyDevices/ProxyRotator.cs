...
namespace AlpacaSpy.ProxyDevices
{
    public class ProxyRotator : IRotatorV4
    {
        ...
        private AlpacaRotator Client => _client ?? throw new NotConnectedException($"Not connected to {_config.Name}");

        public ProxyRotator(ConfiguredDevice config, State state, AlpacaSpyLogger logger)
        {
            _config = config;
            _state = state;
            _logger = logger;
        ...
        internal void ConnectDevice()
        {
            ...
            DisconnectDevice();
            _client = new AlpacaRotator(ASCOM.Common.Alpaca.ServiceType.Http, _config.IpAddress, _config.PortNumber, _config.RemoteDeviceNumber, false, null);
        ...
        public bool CanReverse => Client.CanReverse;
        public bool IsMoving => Client.IsMoving;
        public float MechanicalPosition => Client.MechanicalPosition;
        public float Position => Client.Position;
        public bool Reverse { get => Client.Reverse; set => Client.Reverse = value; }
        public float StepSize => Client.StepSize;
        public float TargetPosition => Client.TargetPosition;

        public void Halt() => Client.Halt();
        public void Move(float position) => Client.Move(position);
        public void MoveAbsolute(float position) => Client.MoveAbsolute(position);
        public void MoveMechanical(float position) => Client.MoveMechanical(position);
        public void Sync(float position) => Client.Sync(position);
    }
}
