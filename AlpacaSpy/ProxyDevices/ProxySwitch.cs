...
namespace AlpacaSpy.ProxyDevices
{
    public class ProxySwitch : ISwitchV3
    {
        ...
        public ProxySwitch(ConfiguredDevice config, State state, AlpacaSpyLogger logger)
        {
            _config = config;
            _state = state;
            _logger = logger;
        ...
        internal void ConnectDevice()
        {
            ...
            DisconnectDevice();
            _client = new AlpacaSwitch(ASCOM.Common.Alpaca.ServiceType.Http, _config.IpAddress, _config.PortNumber, _config.RemoteDeviceNumber, false, null);
