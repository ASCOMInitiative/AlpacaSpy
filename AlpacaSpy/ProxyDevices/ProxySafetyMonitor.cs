...
{
    public class ProxySafetyMonitor : ISafetyMonitorV3, IDisposable
    {
        ...
        public ProxySafetyMonitor(ConfiguredDevice config, State state, Settings settings, AlpacaSpyLogger logger)
        {
            _config = config;
            _state = state;
            _settings = settings;
        ...
        internal void ConnectDevice()
        {
            ...
            DisconnectDevice();
            _client = new AlpacaSafetyMonitor(
